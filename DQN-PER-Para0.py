...
buffer = Memory(10000)
pickle.dump(buffer, open("buffer0.data", "wb"))
buffer = pickle.load(open("buffer0.data", "rb"))
loadmodel = False
gamma = 0.95
LEARNING_RATE = 0.001
...
game = hlt.Game()
game.ready("DQN-PER-Para0-GoodMove")
""" <<<Game Loop>>> """
while True:
    game.update_frame()
    me = game.me
    ...
    if game.turn_number == max_turn_number:
        ...
        with open('progress.txt', "a") as myfile:
            myfile.write("\n-------------\nHalite won : " + str(total_halite) + "\n")
            myfile.write("Total reward : " + str(total_reward) +"\n")
            myfile.write("Ships : " + str(len(me.get_ships())) +"\n")
            myfile.write("Reward per iteration : " + str(float(counter)/total_reward) +"\n")
