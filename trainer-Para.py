...
def create_programs(game_number, epsilon):
    ...
    seed = random.randint(0, 16000000)
    if game_number % 50 == 0:
...
for i in range(start, 15000):
    ...
    for _ in range (train_step):
        ...
        model.fit(np.concatenate((batch_states1, batch_states0), axis=0), np.concatenate((batch_target1, batch_target0), axis=0), epochs=1, verbose=0, batch_size=64)
        ...
        model.fit(np.concatenate((batch_states2, batch_states3), axis=0), np.concatenate((batch_target2, batch_target3), axis=0), epochs=1, verbose=0, batch_size=64)
        ...
        model.fit(np.concatenate((batch_states4, batch_states5), axis=0), np.concatenate((batch_target4, batch_target5), axis=0), epochs=1, verbose=0, batch_size=64)
